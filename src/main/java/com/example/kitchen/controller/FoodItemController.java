...
public class FoodItemController {
    ...
    // GET /items/{id}
    @GetMapping("/{id}")
    public ResponseEntity<FoodItemResponse> getOne(@PathVariable Long id) {
        return ResponseEntity.ok(service.getItem(getUsername(), id));
    }

    // GET /items/expiring?days=7
    @GetMapping("/expiring")
    public ResponseEntity<List<FoodItemResponse>> getExpiring(@RequestParam(defaultValue = "7") int days) {
    ...
    public ResponseEntity<List<FoodItemResponse>> getByLocation(@PathVariable FoodItem.Location location) {
        return ResponseEntity.ok(service.getByLocation(getUsername(), location));
    }

    // GET /items/type/{type}  e.g. DAIRY, MEAT, PRODUCE
    @GetMapping("/type/{type}")
    public ResponseEntity<List<FoodItemResponse>> getByType(@PathVariable FoodItem.FoodType type) {
        return ResponseEntity.ok(service.getByFoodType(getUsername(), type));
    }

    // POST /items
    @PostMapping
    public ResponseEntity<FoodItemResponse> create(@RequestBody FoodItemRequest item) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.addItem(getUsername(), item));
    }

    // PUT /items/{id}
    @PutMapping("/{id}")
    public ResponseEntity<FoodItemResponse> update(@PathVariable Long id, @RequestBody FoodItemRequest item) {
        return ResponseEntity.ok(service.updateItem(getUsername(), id, item));
    }

    // PATCH /items/{id}/consume
    @PatchMapping("/{id}/consume")
    public ResponseEntity<FoodItemResponse> consume(@PathVariable Long id) {
